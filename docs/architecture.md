...
Audio routing now has a first-class `AudioService` boundary. The API asks the service for input devices, output devices, route state, selected device IDs, and mixed bus levels, then exposes that state through the same appliance snapshot. The current `MockAudioService` lives in `@studybox/audio` and supports mock device selection through protected API routes; the future Raspberry Pi implementation should satisfy the same interface with real Linux audio discovery and routing.
...
Recordings can expose multiple downloadable assets. The `audio` asset is the local StudyBox room capture, currently recorded from the DJI receiver through ALSA. The `zoom` asset is reserved for the Zoom-generated recording once that integration produces a file. New local recordings default to a 35-day availability window, controlled by `STUDYBOX_RECORDING_RETENTION_DAYS`, so the Pi can keep the last four Saturday meetings plus a short buffer without becoming a long-term archive.
...
The current implementation includes the local queue and a transport boundary. When the Zoom meeting is stopped and the recording is finished, StudyBox writes a session bundle under `STUDYBOX_BACKUP_DIR`, records queue state in `STUDYBOX_BACKUP_QUEUE_PATH`, and syncs pending bundles. Local development defaults to mock sync under `data/backup-bundles/`; Hetzner stages Church of the Word bundles under `/srv/studybox-backups/churchoftheword/stage` and promotes successful uploads into `/srv/studybox-backups/churchoftheword/completed`; the Pi uses `/var/lib/studybox/backups` as its local queue before pushing to Hetzner.

For the Pi, set `STUDYBOX_BACKUP_MODE=rsync` and provide `STUDYBOX_BACKUP_HOST`, `STUDYBOX_BACKUP_USER`, `STUDYBOX_BACKUP_REMOTE_DIR`, `STUDYBOX_BACKUP_SSH_KEY`, and optionally `STUDYBOX_BACKUP_PORT`. When `STUDYBOX_BACKUP_STAGE_REMOTE_DIR` is set, the rsync uploader writes each bundle into the stage directory first and promotes it into the completed directory only after rsync succeeds. Failed uploads remain retryable and do not block meeting controls.
